...
export const createToken = async(
    ...
    const ybnftContract = new ethers.Contract(
        addresses[networkID].YBNFT_ADDRESS as string,
        YBNFTAbi,
        provider,
    );
    
    try {
        const tx = await ybnftContract.createToken(
            percents,
            tokens,
            stakeAddresses
        );
...
export const depositOnNft = async(
    ...
    try {
        const tx = await ybnftContract.deposit(
            tokenId,
            amunt,
            tokenAddress
        );
