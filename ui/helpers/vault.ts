...
export const getInfo = async(
    ...
    const vaultContract = new ethers.Contract(
        addresses[chainId].VAULT_ADDRESS as string,
        VaultABI,
        provider,
    );

    const tokenContract = new ethers.Contract(
        addresses[chainId].MOCLUP_ADDRESS as string,
        MockABI,
        provider
    );
    ...
    const userStake = address.length > 0 ? (await vaultContract.userStake(address, addresses[chainId].MOCLUP_ADDRESS)) : { reward: 0 };
...
export const vaultAction = async(
    provider: JsonRpcProvider,
    token: string,
    amount: BigNumber,
    action: string
