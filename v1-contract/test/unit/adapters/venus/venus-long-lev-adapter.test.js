...
describe("VenusLongLevAdapter Unit Test", function () {
  before("Deploy contract", async function () {
    ...
    const VenusAdapter = await ethers.getContractFactory("VenusLongLevAdapter");
    this.aAdapter = await VenusAdapter.deploy(this.strategy, "Venus::BUSD Long Leverage Adapter");
  ...
  describe("should increase withdrawal amount from only investor", function () {
    it("revert not from investor", async function () {
      await expect(
        this.aAdapter.connect(this.alice).increaseWithdrawalAmount(this.alice.address, 1, ethers.utils.parseUnits("1"))
    ...
    it("test increaseWithdrawalAmount functinon", async function () {
      await this.aAdapter
        ...
        .increaseWithdrawalAmount(this.alice.address, 1, ethers.utils.parseUnits("1"));
  ...
  describe("should set investor only from owner wallet", function () {
    it("revert when from owner wallet", async function () {
      await expect(this.aAdapter.connect(this.alice).setInvestor(this.owner.address)).to.be.revertedWith(
