...
describe("YBNFT capability for editing strategy Test", function () {
  before("Deploy contract", async function () {
    ...
    const AaveFarmAdapter = await ethers.getContractFactory("AaveLendAdapterEth", {
      libraries: {
    ...
    // Deploy two adapters
    this.adapter1 = await AaveFarmAdapter.deploy(strategy, usdc, aUSDC, this.sushiRouter, "AAVE::Lend::USDC", weth);
    await this.adapter1.deployed();
    this.adapter2 = await AaveFarmAdapter.deploy(strategy, usdc, aUSDC, this.sushiRouter, "AAVE::Lend::USDC", weth);
    ...
    await this.adapter1.setPath(this.weth, this.usdc, [this.weth, this.usdc]);
    await this.adapter1.setPath(this.usdc, this.weth, [this.usdc, this.weth]);
    await this.adapter2.setPath(this.weth, this.usdc, [this.weth, this.usdc]);
    await this.adapter2.setPath(this.usdc, this.weth, [this.usdc, this.weth]);
  ...
  describe("Check init variables", function () {
    it("(1) check performance fee", async function () {
      expect(await this.ybNft.performanceFee(1)).to.eq(100) && expect(await this.ybNft.performanceFee(2)).to.eq(100);
    });
  ...
  describe("Check capabilities to edit strategy", function () {
    it("(1) test capability to update performance fee", async function () {
      await expect(this.ybNft.updatePerformanceFee(1, 1000)).to.be.revertedWith(
      ...
      expect(await this.ybNft.performanceFee(1)).to.eq(500) && expect(await this.ybNft.performanceFee(2)).to.eq(800);
    });

    it("(2) test capability to update adaper allocation", async function () {
      await expect(this.ybNft.updateAllocations(1, [1000, 10000])).to.be.revertedWith("Incorrect adapter allocation");
    ...
    it("(3) check tokenURI", async function () {
      await expect(this.ybNft.updateTokenURI(3, "tokenURI3")).to.be.revertedWith(
