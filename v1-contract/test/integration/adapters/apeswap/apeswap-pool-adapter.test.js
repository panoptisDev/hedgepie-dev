...
describe("ApeswapPoolAdapter Integration Test", function () {
  before("Deploy contract", async function () {
    ...
    // Mint NFTs
    // tokenID: 1
    await this.ybNft.mint([10000], [Banana], [this.aAdapter.address], performanceFee, "test tokenURI1");

    // tokenID: 2
    await this.ybNft.mint([10000], [Banana], [this.aAdapter.address], performanceFee, "test tokenURI2");
  ...
  describe("depositBNB function test", function () {
    ...
    it("(2)should be reverted when caller is not matched", async function () {
      ...
      const depositAmount = ethers.utils.parseEther("1");
      await expect(
        this.investor.depositBNB(this.aliceAddr, 1, depositAmount.toString(), {
          gasPrice: 21e9,
          value: depositAmount,
        })
  ...
  describe("withdrawBNB() function test", function () {
    ...
    it("(2)should be reverted when caller is not matched", async function () {
      // deposit to nftID: 1
      await expect(this.investor.withdrawBNB(this.aliceAddr, 1, { gasPrice: 21e9 })).to.be.revertedWith(
        "Error: Caller is not matched"
      );
    });

    it("(3)should receive BNB successfully after withdrawBNB function", async function () {
    ...
    it("(4)should receive BNB successfully after withdrawBNB function for bob", async function () {
      // withdraw from nftId: 1
    ...
    it("(5)should receive BNB successfully after withdrawBNB function for tom", async function () {
      // withdraw from nftId: 1
