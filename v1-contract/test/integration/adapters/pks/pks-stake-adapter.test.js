...
describe("Pancakeswap Stake Adapter Integration Test", function () {
  before("Deploy contract", async function () {
    ...
    await this.pksStakeAdapter.setPath(wbnb, this.stakingToken, [wbnb, this.stakingToken]);
    await this.pksStakeAdapter.setPath(this.stakingToken, wbnb, [this.stakingToken, wbnb]);
  ...
  describe("should set correct state variable", function () {
    ...
    it("(6)Check AdapterInfo of YBNFT", async function () {
      ...
      expect(response[0].token.toLowerCase()).to.eq(this.stakingToken.toLowerCase());
  ...
  describe("depositBNB function test", function () {
    ...
    it("(4)deposit should success for Alice", async function () {
      ...
      const aliceAdapterInfos = await this.investor.userAdapterInfos(this.aliceAddr, 1, this.pksStakeAdapter.address);
      expect(BigNumber.from(aliceAdapterInfos.amount).gt(0)).to.eq(true);

      const adapterInfos = await this.investor.adapterInfos(1, this.pksStakeAdapter.address);
      expect(BigNumber.from(adapterInfos.totalStaked).sub(BigNumber.from(aliceAdapterInfos.amount))).to.eq(0);

      const aliceWithdrable = await this.pksStakeAdapter.getWithdrawalAmount(this.aliceAddr, 1);
    ...
    it("(5)deposit should success for Bob", async function () {
      const aliceAdapterInfos = await this.investor.userAdapterInfos(this.aliceAddr, 1, this.pksStakeAdapter.address);
      const beforeAdapterInfos = await this.investor.adapterInfos(1, this.pksStakeAdapter.address);
      ...
      const bobAdapterInfos = await this.investor.userAdapterInfos(this.bobAddr, 1, this.pksStakeAdapter.address);
      expect(BigNumber.from(bobAdapterInfos.amount).gt(0)).to.eq(true);

      const afterAdapterInfos = await this.investor.adapterInfos(1, this.pksStakeAdapter.address);
      ...
      const bobWithdrable = await this.pksStakeAdapter.getWithdrawalAmount(this.bobAddr, 1);
    ...
    it("(6)deposit should success for Tom", async function () {
      ...
      const tomAdapterInfos = await this.investor.userAdapterInfos(this.tomAddr, 1, this.pksStakeAdapter.address);
      ...
      const tomWithdrable = await this.pksStakeAdapter.getWithdrawalAmount(this.tomAddr, 1);
  ...
  describe("withdrawBNB() function test", function () {
    ...
    it("(3)should receive BNB successfully after withdraw function for Alice", async function () {
    ...
    it("(4)should receive BNB successfully after withdraw function for Bob", async function () {
      // withdraw from nftId: 1
    ...
    it("(5)should receive BNB successfully after withdraw function for Tom", async function () {
      // withdraw from nftId: 1
