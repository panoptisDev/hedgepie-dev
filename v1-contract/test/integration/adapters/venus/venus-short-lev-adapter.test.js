...
describe("VenusShortLevAdapter Integration Test", function () {
  before("Deploy contract", async function () {
    ...
    const VenusShotLevAdapter = await ethers.getContractFactory(
      "VenusShortLevAdapter"
    );
    this.aAdapter = await VenusShotLevAdapter.deploy(
      this.vBusd.address,
  ...
  describe("depositBNB function test", function () {
    ...
    it("(4) deposit should success for Alice", async function () {
      ...
      console.log(
        ...
        BigNumber.from(
          await this.busdToken.balanceOf(this.investor.address)
        ).toString()
      );
