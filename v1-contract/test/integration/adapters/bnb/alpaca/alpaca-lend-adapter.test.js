...
describe("AlpacaLendAdapter Integration Test", function () {
  before("Deploy contract", async function () {
    ...
    this.aAdapter = await AlpacaAdapter.deploy(strategy, stakingToken, stakingToken, repayToken, "Alpaca:: BNB Lend");
  ...
  describe("depositBNB function test", function () {
    ...
    it("(4) deposit should success for Bob", async function () {
      ...
      expect(BigNumber.from(bobWithdrable)).to.eq(
        BigNumber.from(afterRepayBalance).sub(BigNumber.from(beforeRepayBalance))
      );
    }).timeout(50000000);
