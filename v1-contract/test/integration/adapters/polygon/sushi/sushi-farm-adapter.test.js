...
describe("SushiSwapFarmLPAdapter Integration Test", function () {
  before("Deploy contract", async function () {
    ...
    const SushiLPAdapter = await ethers.getContractFactory("SushiSwapLPAdapter");
    ...
    await this.aAdapter.setInvestor(this.investor.address);
    await this.aAdapter.setPath(this.wmatic, this.sushi, [this.wmatic, this.sushi]);
    await this.aAdapter.setPath(this.sushi, this.wmatic, [this.sushi, this.wmatic]);
    await this.aAdapter.setPath(this.wmatic, this.usdc, [this.wmatic, this.usdc]);
    await this.aAdapter.setPath(this.usdc, this.wmatic, [this.usdc, this.wmatic]);
    await this.aAdapter.setPath(this.wmatic, this.dai, [this.wmatic, this.dai]);
    await this.aAdapter.setPath(this.dai, this.wmatic, [this.dai, this.wmatic]);
  ...
  describe("depositMATIC function test", function () {
    ...
    it("(2) should be reverted when amount is 0", async function () {
      ...
      await expect(
        this.investor.depositMATIC(this.owner.address, 1, depositAmount.toString(), { gasPrice: 21e9 })
  ...
  describe("withdrawMATIC() function test", function () {
    it("(1) revert when nft tokenId is invalid", async function () {
      ...
      // withdraw to nftID: 3
      await expect(this.investor.withdrawMATIC(this.owner.address, 3, { gasPrice: 21e9 })).to.be.revertedWith(
    ...
    it("(2) should receive the MATIC successfully after withdraw function for Alice", async function () {
      ...
      await expect(this.investor.connect(this.alice).withdrawMATIC(this.aliceAddr, 1, { gasPrice: 21e9 })).to.emit(
        this.investor,
        "WithdrawMATIC"
      );
    ...
    it("(4) should receive the MATIC successfully after withdraw function for Bob", async function () {
      ...
      await expect(this.investor.connect(this.bob).withdrawMATIC(this.bobAddr, 1, { gasPrice: 21e9 })).to.emit(
        this.investor,
        "WithdrawMATIC"
      );
