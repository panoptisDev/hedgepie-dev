...
import { venusBUSDLendAdapterArgs, pksGalStakeAdapterArgs } from "../config/construct-arguments";
...
async function deploy() {
  ...
  const venusLendAdapter = await hre.ethers.getContractFactory("VenusLendAdapter");
  ...
  const pksGalStakeAdapter = await hre.ethers.getContractFactory("PancakeStakeAdapter");
...
async function main() {
  const { venusBUSDLendAdapter, pksGalStakeAdapter, ybnft, investor, adapterManager } = await deploy();
  ...
  await verify({
    ...
    contractPath: "contracts/adapters/venus/venus-lend-adapter.sol:VenusLendAdapter",
  });
  ...
  await verify({
    ...
    contractPath: "contracts/adapters/pancakeswap/pancake-stake-adapter.sol:PancakeStakeAdapter",
  });
