...
const venusAdapterArgs = venusAdapterArgsList["Venus::Lend::BUSD"];

async function deploy() {
    // deploy Venus lend adapter contract
    ...
    const venusAdapter = await venusAdapterFactory.deploy(
        ...
        venusAdapterArgs.stakingToken,
        venusAdapterArgs.repayToken,
        venusAdapterArgs.name
    );
    await await venusAdapter.deployed();
    const venusAdapterAddress = venusAdapter.address;
    log.info(
        `VenusLendAdapter contract was successfully deployed on network: ${hre.network.name}, address: ${venusAdapterAddress}`
    );
...
async function main() {
    ...
    // verify Venus lend adapter contract
    await verify({
        contractName: "VenusLendAdapter",
        address: venusAdapter,
        constructorArguments: Object.values(venusAdapterArgs),
        contractPath:
            "contracts/adapters/bnb/venus/venus-lend-adapter.sol:VenusLendAdapter",
    });
}
