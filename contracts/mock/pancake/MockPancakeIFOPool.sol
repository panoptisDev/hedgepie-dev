...
contract MockPancakeIFOPool is Ownable, Pausable {
    ...
    constructor(
        ...
        require(
            ...
            "start block can't behind current block"
        );
    ...
    function deposit(uint256 _amount) external whenNotPaused notContract {
        ...
        require(
            currentShares > 0,
            "deposit amount is too small to allocate shares"
        );
    ...
    function _calculateAvgBalance(
        ...
        uint256 lastContribute = _lastAvgBalance.mul(
            _lastValidActionBlock.sub(startBlock)
        );
        uint256 currentContribute = _lastValidActionBalance.mul(
            currentBlock.sub(_lastValidActionBlock)
        );
        avgBalance = (lastContribute.add(currentContribute)).div(
            currentBlock.sub(startBlock)
        );
    }
     ...
     */
    function getUserCredit(address _user)
        external
        view
    ...
    function withdraw(uint256 _shares) public notContract {
        ...
        if (bal < currentAmount) {
            ...
            IPancakeFarmMasterChef(masterchef).leaveStaking(balWithdraw);
    ...
    function harvest() external notContract whenNotPaused {
        ...
        IPancakeFarmMasterChef(masterchef).leaveStaking(0);
        ...
        onlyAdmin
    {
        require(
            block.number < _startBlock,
            "Pool current block must be lower than new startBlock"
        );
        require(
    ...
    function updateEndBlock(uint256 _endBlock) external onlyAdmin {
        require(
            ...
            "new end block can't behind current block"
        );
        require(
            block.number < endBlock,
            "old end block can't behind current block"
        );
    ...
    function emergencyWithdraw() external onlyAdmin {
        IPancakeFarmMasterChef(masterchef).emergencyWithdraw(0);
        if (!paused()) {
            _pause();
        }
    }
    ...
    function calculateHarvestCakeRewards() external view returns (uint256) {
        uint256 amount = IPancakeFarmMasterChef(masterchef).pendingCake(
            0,
            address(this)
        );
    ...
    function balanceOf() public view returns (uint256) {
        (uint256 amount, ) = IPancakeFarmMasterChef(masterchef).userInfo(
            0,
            address(this)
        );
    ...
    function _earn() internal {
        uint256 bal = available();
        if (bal > 0) {
            IPancakeFarmMasterChef(masterchef).enterStaking(bal);
        }
    }
